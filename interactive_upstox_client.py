...
class UpstoxClient:
    ...
    async def connect(self):
        ...
        server_params = StdioServerParameters(
            command="python", args=["main.py", "--server", "upstox"]
        )
    ...
    async def get_market_data(self, symbols: list):
        """Get market data"""
        result = await self.session.call_tool("get_market_data", {"symbols": symbols})
        return json.loads(result.content[0].text)
    ...
    try:
        ...
        while True:
            ...
            if choice == "1":
                ...
                order_type = (
                    input("Enter order type (MARKET/LIMIT) [MARKET]: ").strip().upper()
                    or "MARKET"
                )
                price = 0
                if order_type == "LIMIT":
                ...
                product = (
                    input("Enter product type (D/I/M) [D]: ").strip().upper() or "D"
                )

                print(f"\n🛒 Placing BUY order for {quantity} shares of {symbol}...")
                result = await client.buy_stock(
                    symbol, quantity, order_type, price, product
                )
                ...
                result = await client.sell_stock(
                    symbol, quantity, order_type, price, product
                )
                print(json.dumps(result, indent=2))
                ...
                quantity = int(input("Enter quantity: ").strip())
                transaction_type = (
                    input("Enter transaction type (BUY/SELL): ").strip().upper()
                )
                order_type = (
                    input("Enter order type (MARKET/LIMIT) [MARKET]: ").strip().upper()
                    or "MARKET"
                ...
                result = await client.place_amo(
                    symbol, quantity, transaction_type, order_type, price
                )
                print(json.dumps(result, indent=2))
                ...
                order_id = (
                    input("Enter order ID (leave blank for all orders): ").strip()
                    or None
                )
